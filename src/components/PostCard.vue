  ...
  <div class="post-card">
    ...
    <div class="post-actions">
      <div class="vote-container">
        <button
          class="vote-button"
          @click="handleVoteClick('upvote', clickedPostId)"
        >
          <svg
            width="24"
            height="24"
        ...
        <div class="post-upvotes">Upvotes: {{ post.votes }}</div>
      </div>

      <div class="vote-container">
        <button
          class="vote-button"
          @click="handleVoteClick('downvote', clickedPostId)"
        >
          <svg
            width="24"
            height="24"
        ...
        <div class="post-downvotes">Downvotes: {{ post.votes }}</div>
      </div>
    </div>

    <div class="post-comments">
      Comments:
      <button class="show-comments-button" @click="toggleComments(post)">
        <img
          class="comment-vector"
          src="https://www.svgrepo.com/show/522072/comment-4.svg"
        />
      </button>
...
export default {
  ...
  methods: {
    ...
    async handleVoteClick(voteType) {
      const postId = this.post._id; // Correctly get the Post ID
      const userId = this.$store.state.user
        ? this.$store.state.user.username
        : null; // Correctly get the User ID
      this.logVoteInfo(voteType, postId); // Pass the correct parameters
      ...
      try {
        const response = await axios.post(
          `http://localhost:5000/vote/${this.post._id}`,
          {
            user: userId,
            vote_type: voteType,
            date: dateFormatted,
        ...
        if (voteType === "upvote") {
          this.$set(this.post, "upvotes", this.post.upvotes + 1); // post verisini güncelle
        } else if (voteType === "downvote") {
          this.$set(this.post, "downvotes", this.post.downvotes + 1); // post verisini güncelle
        }
        ...
        if (error.response.status === 401) {
          ...
          alert("Voting request failed. Please try again later.", error);
        }
      }
    },
    ...
    async addComment(post) {
      ...
      try {
        const response = await axios.post(
          `http://localhost:5000/add_comment/${post._id}`,
          {
        ...
        if (error.response.status === 401) {
          ...
          alert("Comment submission failed. Please try again later.", error);
        }
      }
    },
  },
...
<style src="../styles/PostCard.css" scoped></style>
